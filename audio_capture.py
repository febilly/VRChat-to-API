...
import numpy as np
from config import MIX_OWN_VOLUME, MIX_OTHER_VOLUME, LOOPBACK_DEVICE_NAME
...
class AudioStreamer:
    ...
    def _create_recorder(self, source: str):
        ...
        try:
            ...
            if source == "system":
                # Optionally target a specific output device by (partial) name so
                # only that device's audio is captured (e.g. route VRChat to a
                # dedicated/virtual output to isolate it). Falls back to default.
                speaker = None
                if LOOPBACK_DEVICE_NAME:
                    try:
                        speaker = sc.get_speaker(LOOPBACK_DEVICE_NAME)
                    except Exception:
                        speaker = None
                    if speaker is None:
                        print(
                            f"⚠️  Loopback device '{LOOPBACK_DEVICE_NAME}' not found; "
